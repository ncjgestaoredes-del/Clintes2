...
const App: React.FC = () => {
  ...
  // Carregar dados da API real (MySQL)
  ...
  const handleAddCustomer = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        await fetchCustomers(); // Atualiza a lista vinda do banco
    ...
    <div className="flex min-h-screen bg-slate-50">
      <aside className="w-64 bg-white border-r border-slate-200 p-6 flex flex-col gap-8 hidden md:flex">
        ...
        <div className="mt-auto bg-slate-900 rounded-2xl p-4 text-white">
          <p className="text-xs text-slate-400 mb-1">Conexão Ativa</p>
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 ${loading ? 'bg-amber-400 animate-pulse' : 'bg-green-400'} rounded-full`}></div>
            <span className="text-sm font-medium">MySQL (AlwaysData)</span>
          </div>
        </div>
      </aside>
              ...
              <div className="space-y-8 animate-in fade-in duration-500">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <p className="text-slate-500 text-sm">Total a Receber (SQL)</p>
                    <h2 className="text-3xl font-bold text-slate-900">R$ {stats.totalReceivable.toLocaleString('pt-BR')}</h2>
                  </div>
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <p className="text-slate-500 text-sm">Base de Clientes</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                    <h3 className="font-bold text-lg mb-6">Últimos Registros do MySQL</h3>
      ...
      {/* Modals permanecem similares, mas com triggers para a API */}
      {isAddingCustomer && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl animate-in zoom-in-95">
            <h2 className="text-2xl font-bold mb-6">Novo Registro MySQL</h2>
            <form onSubmit={handleAddCustomer} className="space-y-4">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button type="submit" className="flex-1 py-3 bg-indigo-600 text-white font-bold rounded-xl shadow-lg shadow-indigo-200">Gravar no Banco</button>
              </div>
            </form>
          </div>
        </div>
